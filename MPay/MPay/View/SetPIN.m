...
- (void)UI {
    ...
    UIButton *showKeyboard = [[UIButton alloc] initWithFrame:CGRectMake(0, textFields.frame.origin.y + 10, self.view.frame.size.width - (openPINS.frame.size.width + 15), 50)];
    ...
    confBTN = [self UIButton:self withFrame:CGRectMake(btnmarginleft, ([self view].frame.size.height/2)-50, self.view.frame.size.width-(btnmarginleft*2), btnheight) withTitle:[@"setMyPin" uppercaseString] withTag:1];
...
//            [self RequestAPIValidatePINBackOffice];
            [self performSegueWithIdentifier:@"SetPINConf" sender:self];
        }
    }
}

-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    NSMutableDictionary *pin = [NSMutableDictionary dictionaryWithObjectsAndKeys:[res text], @"PIN", nil];
    if ([[segue identifier] isEqualToString:@"SetPINConf"]) {
        SetPINConf *NVC = [segue destinationViewController];
        NVC.pin = pin;
        NVC.data = data;
...
-(void)textFieldDidBeginEditing:(UITextField *)textField {
    ...
    [confBTN setFrame:CGRectMake(10, textFields.frame.size.height + textFields.frame.origin.y + [self view].frame.size.height/10, self.view.frame.size.width - 20, btnheight)];
    for (id aSubview in bgview.subviews){
