...
#import "OTPForgotPIN.h"
#import "Login.h"

@implementation OTPForgotPIN
{
...
- (void)UI {
    ...
    [label setText:[NSString stringWithFormat:@"forgotPINDesc%@", [[response objectForKey:@"params"] objectForKey:@"mobileNumber"]]];
    ...
    UIButton *resendBtn = [self UIButton:self withFrame:CGRectMake(20, sendBtn.frame.size.height + sendBtn.frame.origin. y + [self view].frame.size.height/50, self.view.frame.size.width - 40, 50) withTitle:@"resendVerificationCode" withTag:1];
...
//                [self RequestAPIValidateOTPByCust];
                [self performSegueWithIdentifier:@"SetNewPIN" sender:self];
            }
        } else {
...
//        [self RequestData:self withAction:@"RequestOTPByCust" withParams:[NSDictionary dictionaryWithObjectsAndKeys:DEVICETOKEN, @"deviceToken", @"Y", @"resendFlag", @"forgotPin", @"view", nil]];
...
//
//    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
//        [self RequestData:self withAction:@"ValidateOTPByCust" withParams:[NSDictionary dictionaryWithObjectsAndKeys:DEVICETOKEN, @"deviceToken", [res text], @"otp", @"forgotPin", @"view", nil]];
